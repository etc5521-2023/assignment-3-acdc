...
tourism |> filter(region == "Ashburton (WA)",
                  purpose == "Holiday") |>
...
```{r, message = FALSE, warning = FALSE}
# Narrow down the most-observed sighting areas
wmm_tourism_nr <- wmm_tourism %>% 
  filter(Purpose %in% c("Holiday")) %>% 
  filter(lga %in% c("Broome ", "Port Hedland ", "Ashburton ", "Central Desert ", "Barkly "))
```

```{r}
packages <- c("tidyverse","here","lubridate","GGally","tsibble","forcats","cartogram","sf","cartogram","patchwork","ggthemes", "sugarbag", "viridis", "rmapshaper", "cubble")

lapply(packages, library, character.only = TRUE)

wmm_tourism_tot <- wmm_tourism_nr %>% 
  group_by(Purpose, lga, Year_Quarter, Year) %>% 
  summarise(tot_trip = sum(Trips))

ggplot(wmm_tourism_tot, aes(Year_Quarter, tot_trip, color = lga))+
  geom_line() +
  labs(x = "Quarter of Year", 
       y = "Total trips")+
...
```{r}
ggplot(wmm_tourism_nr) +
  geom_sf(aes(fill = lga))+
  theme_map()
```

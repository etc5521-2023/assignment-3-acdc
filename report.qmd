...
time_series_plot <- wmm_tourism_dg %>%
  filter(region == "Barkly") %>%
  group_by(Year_Quarter) %>%
  summarise(Total_Trips = sum(Trips, na.rm = TRUE)) %>%
  ggplot(aes(x = Year_Quarter, y = Total_Trips)) +
  geom_line(linewidth = 1, color = "aquamarine4", alpha = 0.5) +
  geom_point(size = 2, color = "maroon", aes(text = paste("Year & Quarter: ", Year_Quarter, "<br>Total Trips ('000): ", Total_Trips))) +
...
qtr_tourism <- quarter_series_plot <- wmm_tourism_dg %>%
                filter(region == "Barkly") %>%
                group_by(Quarter) %>%
                summarise(Total_Trips = sum(Trips, na.rm = TRUE)) %>%
                ...
                geom_bar(position = "dodge", 
                         stat = "identity", 
                         fill = "aquamarine4",
                         aes(text = paste("Quarter: ", Quarter, "<br>Total Trips ('000): ", Total_Trips))) +
                #geom_line(linewidth = 1, color = "aquamarine4", alpha = 0.5) +
...
- The left graph shows the number of tourism for each quarter, it clearly shows that Q3 has the highest number of tourism with more than 1.500('000) people travelling into Barkly, followed by Q2 with more than 500('000) people, Q4 with above 250('000) people, and last, Q1 with lower than 100('000) people. 
...
temporal_plot <- wmm_tourism_dg %>%
                  filter(region == "Barkly") %>%
                  group_by(Year_Quarter, Purpose) %>%
                  summarise(sum_trips = sum(Trips)) %>%
                  ggplot(aes(x = Year_Quarter, y = sum_trips)) +
                  geom_line(size = 1) +
                  labs(

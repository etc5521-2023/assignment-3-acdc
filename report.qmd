...
(sightings), it will be best to join these two datasets. By joining
these two datasets, we will be able to perform temporal and spatial
...
are going to break down the `eventDate` variable into year & quarter
format.
...
Now, our Occurence data has matching region name with the region in the
...
those expectations through visual representation in order to help faster
the process of understanding. Each of the analysis will be completed
with discussion points that extracted from our analysis.
...
ggplot(lineup(null_permute('sightings_tot'), wmm_tourism_holiday), aes(x = Trips, y = sightings_tot)) +
              ...
              theme(axis.text = element_blank(), 
                    panel.border = element_rect(fill=NA, colour="black"))
```
Null Hypothesis: There is no relationship between number of tourist visiting a certain region to the number of sightings of Weeping Mulla Mulla in that same region.
...
From the computation, the p-value of correlation plot is 0.401 or 40.1%. Since it is more than 0.05 or 5% (significance level), then we will accept H0. This mean there is weak linear relationship between number of tourism and number of sightings. The majority of the participants in the survey expressed that identifying the most distinct plot from the selection is quite challenging due to the similarity in patterns among all of them.

...
-   We expect the sightings to be consistent numbers of sightings with
    ...
    better. However, this anticipation will exclude years during the COVID-19 pandemic. Since the data relies on human observation, there may be fewer sightings due to a lack of reporting.
...
Throughout this exploration of expectations, numerous intriguing observations were uncovered that were not initially anticipated. Notably, the employment of `spData` and `leaflet` effectively visualised these findings.

Several limitations have impacted the final analysis. Firstly, there is a scarcity of observations, primarily because this plant is not commonly sought after or reported by individuals. Secondly, the absence of weather recordings in remote areas may result in gaps in the dataset. Additionally, extrapolating weather data from one region to the entire species range, instead of relying on the actual weather conditions recorded in each area, could introduce inaccuracies into the analysis.
...
**Joining the species and weather data set**

The final step is to integrate the temperature data back into the species observations, thereby creating a comprehensive dataset that encompasses both observations and temperature data.

```{r}
temperature <- weeping_mulla_mulla %>%
  left_join(tennant_creek_airport, by = c("eventDate" = "date")) %>%
  select(latitude:Day, prcp:tmin) %>% 
  mutate(sighting = ifelse(is.na(tmax) & is.na(tmin), 0, 1)) 
...
### Websites
 ...
 - **The Atlas of Living Australia**. Ptilotus calostachyus F.Muell.Retrieved September 27, 2023, from https://bie.ala.org.au/species/https://id.biodiversity.org.au/node/apni/2910906#
 overview

...
plains (Florabase n. d.). Being a *perennial* plant, it can live up to
more than two years with the seasonal pattern of being dormant in the
winter and sprouting again in springtime (Florabase n. d.). Moreover,
**Weeping Mulla Mulla** displays erect or spreading, herb or shrub
...
The basis of record was filtered to `HUMAN_OBSERVATION` only. The intention of this analysis is to focus solely on sightings of
the species in the wild, and differentiate between specimens that are
...
tourism_tsibble <- tourism %>%
  mutate(Quarter = as.Date(Quarter, format = "%YYYY Q%q")) 

tourism_subset <- tourism_tsibble %>% 
  left_join(weeping_mulla_mulla, by = c("Region" = "lga")) %>% 
  filter(Region %in% c("Barkly", "Tennant Creek", "Tanami", "Ashburton", "Port Hedland", "Broome"))

# Tennant Creek belongs to Barkly, Tanami belong to Central Desert
```
...
Similarly, we will be using `visdat` library to check for data type errors
...
From the @fig-plots98, we can see that there are some NAs in `prcp`, `tmax`, and `tmin` columns. These empty data observations will then be removed accordingly.

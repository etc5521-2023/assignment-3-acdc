...
# Filter out "Total" and NA values in the "Purpose" column
filtered_data <- wmm_tourism %>%
  filter(!Purpose %in% c("Total", NA))

# Group by Year_Quarter and Purpose, and calculate the total trips
temporal_analysis <- filtered_data %>%
  group_by(Year_Quarter, Purpose) %>%
  summarise(Total_Trips = sum(Trips, na.rm = TRUE)) %>%
  ungroup()

# Calculate the moving average and seasonal component
temporal_analysis <- temporal_analysis %>%
  group_by(Purpose) %>%
  mutate(Moving_Average = zoo::rollmean(Total_Trips, k = 4, fill = NA)) %>%
  mutate(Seasonal_Component = Total_Trips - Moving_Average) %>%
  ungroup()

# Create a plot to visualize the seasonal trend
ggplot(temporal_analysis, aes(x = Year_Quarter, y = Seasonal_Component, color = Purpose)) +
  geom_line() +
  labs(title = "Seasonal Trend Analysis by Purpose",
       x = "Year and Quarter",
...
# Create a time series plot for the "Trips" variable
wmm_tourism %>%
  group_by(Year_Quarter) %>%
  summarise(Total_Trips = sum(Trips)) %>%
  ggplot(aes(x = Year_Quarter, y = Total_Trips)) +
  geom_line() +
  labs(title = "Tourist Trips Over Time",
       x = "Year and Quarter",
       y = "Total Trips") +
  theme_minimal()
...
wmm_tourism %>%
  group_by(Year_Quarter, Purpose) %>%
  summarise(Total_Trips = sum(Trips)) %>%
  ggplot(aes(x = Year_Quarter, y = Total_Trips, color = Purpose)) +
  geom_line() +
  labs(title = "Tourist Trips Over Time by Purpose",
       x = "Year and Quarter",
       y = "Total Trips",
       color = "Purpose") +
  theme_minimal()

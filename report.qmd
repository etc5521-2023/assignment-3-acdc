...
wmm_tourism_dg <- wmm_tourism %>% st_drop_geometry()
```
...
- The plot ... shows the total number of tourist trips over time, grouped by year and quarter. As can be seen from the trend, there is a sudden and significant jump of number of tourism visiting Barkly region in Q3 2015. The numbers drop significantly after that particular period and continued the trend similar as before Q3 2015. After doing some research, there is no news, articles, or other type of informations that explained such uncommon trend.
...
qtr_temp <- tca_cb %>% face_temporal() %>%
              ...
              ggplot(aes(x = date, y = avg_tmp, group = 1)) +
                geom_line(size = 1, color = "aquamarine4") +
...
wmm_tourism_holiday <- wmm_tourism %>% 
  filter(Purpose %in% c("Holiday"),
         region %in% c("Barkly", "Tanami", "Victoria River"))
```
...
ggplot(wmm_tourism_tot, aes(Year_Quarter, tot_trip))+
  geom_line() +
...
wmm_tourism_holiday %>%
  ...
  labs(fill = "The total of people visit (x1000)", size = "The number of occurences recorded")
```

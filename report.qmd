...
- The plot shows the total number of tourist trips over time, grouped by year and quarter. There is a clear seasonal pattern, with more trips occurring in the summer months (Q1 and Q2). There is also a slight increasing trend over time.

- The seasonal pattern is most likely due to a number of factors, including school holidays, summer vacations, and favorable weather conditions. The increasing trend over time is likely due to a combination of factors, including rising incomes, increasing globalization, and the growing popularity of air travel.

#### Temporal Trends by Quarter

```{r trend, message = false, warning = false}

quarter_series_plot <- wmm_tourism %>%
  filter(region == "Barkly") %>%
  group_by(Quarter) %>%
  summarise(Total_Trips = sum(Trips, na.rm = TRUE)) %>%
...
interactive_quarter <- ggplotly(quarter_series_plot, tooltip = "text") %>%
                                  layout(title = list(y = .925, 
                                                      xref = "plot", 
                                                      text = paste0("Tourist Trips Over Time (Summarised per Quarter)",
                                                                    "<br>",
                                                                    "<sup>",
                                                                    "Region : Barkly",
                                                                    "<sup>")),
...
#### Temporal Trends by Quarter for Different Purposes and Regions

```{r}

# Create a facetted plot with non-missing 'Purpose' and 'Region' data
facet_by_purpose <- wmm_tourism %>%
  filter(region == "Barkly") %>%
  ggplot(aes(x = Year_Quarter, y = Trips)) +
  geom_line() +
  facet_wrap(~ Purpose, scales = "free") +
  labs(
    title = "Temporal Trends by Quarter for Different Purposes and Regions",
    x = "Year and Quarter",
    y = "Total Trips",
    color = "Region"
  ) +
...
facet_by_purpose_plotly <- ggplotly(facet_by_purpose)

facet_by_purpose_plotly
...
wmm_tourism %>%
  filter(!is.na(Purpose) & Purpose != "Total") %>%
  ...
  geom_smooth(method = "loess", se = FALSE) -> temporal_plot
...
###### Temporal Trends by Quarter for Different Regions

```{r}
# Create a temporal analysis for tourism data
tourism_temporal <- wmm_tourism %>%
  ggplot(aes(x = Year_Quarter, y = Trips, color = region)) +
  geom_line() +
  labs(
    title = "Temporal Trends by Quarter for Different Regions",
    x = "Quarter",
    y = "Trips",
    color = "region"
  ) +
...
tourism_temporal_plotly <- ggplotly(tourism_temporal)

tourism_temporal_plotly
...
We firstly narrow down the necessary data related to Eco-tourism interest of Weeping Mulla Mulla by filtering out Trips with "Holiday" purpose only. We assume that people who join eco-tourism activities must be on their vacation, hence the selection of of `Purpose` variable plays a role here.

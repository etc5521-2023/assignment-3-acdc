...
- The plot ... shows the total number of tourist trips over time, grouped by year and quarter. As can be seen from the trend, .
...
```
- According to the graph ... above, we can see clearly that all purposes have the same peak, which is in Q3 2015. It turns out Barkly region was very popular back then. Moving on to the purposes itself, "Holiday" come out on top as the main reason, followed by "Visiting", "Other", and "Business".  

In the next section, we will utilise all these informations we gather around the tourism trend in Barkly region to compare it with Occurences dataset. We hope to see if there is any specific eco-tourism related to Weeping Mulla Mulla that may have contributed to the significant jump over the number of tourism into Barkly region. 
...
```{r}
grid.arrange(map_p1, map_p2, ncol = 2) 
```

**Discussion**

Expanding on our prior discussion, this map offers an insightful visualisation of the relationship between Weeping Mulla Mulla occurrences and tourism numbers.

The map clearly illustrates a prominent presence of Weeping Mulla Mulla in the North-central areas of Western Australia and parts of the Northern Territory. Notably, Ashburton (WA), situated in the most vivid blue region, contrasts with Barkly, which is depicted in a darker hue representing fewer tourists. This suggests that the ecological significance of these regions, underscored by the substantial number of Weeping Mulla Mulla sightings, might not be the main draw for visitors to these areas.

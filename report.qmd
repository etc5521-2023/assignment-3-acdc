...
#### Temporal analysis of the seasonal patterns to understand if they are increasing or decreasing.

##### General Trend of tourists over time 
...
time_series_plot <- wmm_tourism %>%
  ...
  geom_line() +
  labs(title = "Interactive Tourist Trips Over Time",
       x = "Year and Quarter",
       y = "Total Trips") +
  theme_minimal()

interactive_time_series_plot <- ggplotly(time_series_plot)
...
###### Scatter Plot of Tourist Trips by Region and Purpose

```{r , warning = FALSE , message=FALSE}
library(plotly)

scatter_plot <- wmm_tourism %>%
  filter(!is.na(Purpose) & Purpose %in% c("Business", "Holiday", "Other", "Visiting")) %>%
  group_by(Year_Quarter, region, Purpose) %>%
  ggplot(aes(x = Year_Quarter, y = Trips, color = region)) +
  geom_point() +
  labs(title = "Scatter Plot of Tourist Trips by Region and Purpose",
       x = "Year and Quarter",
       y = "Total Trips",
       color = "Region") +
  theme_minimal() +
...
# Filter out rows with NA values in the 'Purpose' and 'Region' columns
filtered_data <- wmm_tourism %>%
  filter(!is.na(Purpose) & !is.na(region) & Purpose != "Total")

# Create a facetted plot with non-missing 'Purpose' and 'Region' data
facet_by_purpose_region <- filtered_data %>%
  ggplot(aes(x = Year_Quarter, y = Trips, color = region)) +
  geom_line() +
...
facet_by_purpose_region_plotly <- ggplotly(facet_by_purpose_region)

facet_by_purpose_region_plotly
...
wmm_tourism %>%
  ...
  ggplot(aes(x = Year_Quarter, y = Trips, color = Purpose)) +
  geom_line(size = 1) +
  labs(
